...
\begin{document}
    ...
    \paragraph{Pipeline Orchestration.} The pipeline is triggered via timmer trigger (\texttt{ScheduledEchoFetch}) that runs daily at 1 AM CT. The function syncs coach data from Echo, fetches yesterday's conversations, uploads each to staging blob storage, and enqueues ETL messages. A watermark (\texttt{PIPELINEWATERMARK} table) tracks the last successful run timestamp to prevent duplicate processing.
