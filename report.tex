...
\begin{document}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                ymin=0, ymax=118,
                clip=false,
                symbolic x coords={small,gpt5few,gpt5ai,claude},
                xtick=data,
                xticklabels={
                    {\shortstack{Small Models\\{\footnotesize(v1 prompt)}}},
                    {\shortstack{GPT-5-mini\\{\footnotesize(few-shot)}}},
                    {\shortstack{GPT-5-mini\\{\footnotesize(AI-refined)}}},
                    {\shortstack{Claude Sonnet 4.6\\{\footnotesize(reference)}}}
                },
                xticklabel style={align=center, font=\small},
                nodes near coords,
                nodes near coords style={font=\small, yshift=4pt},
                ymajorgrids=true,
                grid style={dashed, gray!40},
            ]
            \addplot[
   ...
   The most visible sign of prompt improvement across iterations was the shift from brief, definition-only tag descriptions to fully specified, example-driven instructions. The prompt went through many incremental revisions, but the most substantive changes occurred across three major versions that each represented a meaningful step in specificity and structure. The earliest version defined each tag in a sentence or two, offering minimal guidance on where the boundary between a flaggable violation and an acceptable coach response actually fell. That ambiguity produced false positives at a rate that stakeholders found unacceptable, and the pattern of misfires made clear that the model was not reliably distinguishing between, for example, a coach sharing a relevant Scripture passage and a coach actually misusing one.
		
		Successive iterations introduced explicit FLAG and NO FLAG examples for each tag. This change had the most direct impact on output quality. By showing the model a paired example of what crosses the threshold and what does not, the prompt gave the evaluator a reference point that definitions alone could not provide. Rationale quality improved noticeably as a result. Earlier outputs would cite a quote and assert a violation without anchoring the reasoning in the broader context of the conversation. After refinement, the model consistently tied its in-text citations to actual conversational context, producing explanations that a human reviewer could follow and verify without re-reading the full transcript.

	Stakeholder feedback, gathered through structured client meetings and email correspondence, was the primary driver of iteration. As reviewers encountered false positive tags, those cases were compared against the prompt definitions and used to tighten the language around specific tags. Some definitions had initially been too broad, applying to situations that were not actually problematic from an operational standpoint. The "Privacy Risk" tag, for example, was refined to explicitly exclude official Groundwire ministry resources such as organizational email addresses and the ministry's outreach number, because earlier runs had incorrectly flagged those as violations. Similarly, the "Difficult Visitor Handling" (or trolling) tag received a clearer engagement threshold, specifying that a coach should not be penalized unless interaction continued well past the point where troll behavior was unambiguous.

	One finding worth noting is that adjustments to score values and label wording had comparatively little effect on output consistency. What mattered more was the specificity of the tag definitions themselves. As those definitions became more detailed and the prompt included concrete examples of both violations and acceptable behavior, model outputs became more stable across conversations, even when label names or scoring conventions shifted between versions. The prompt refinement process ultimately confirmed that behavioral examples are a more reliable anchor for the evaluator than abstract scoring criteria alone.
